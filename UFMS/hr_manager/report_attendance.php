...
if (isset($_GET['employee_id'])) {
    ...
    for ($day = 1; $day <= $days_in_month; $day++) {
        ...
        $full_month_records[$date] = array(
            'created' => 'NotCreated',
            'status' => 'UnMarked'
        );
    }
    
    $attendance_sql = "SELECT date, created, status FROM attendance WHERE employee_id = ? AND DATE_FORMAT(date, '%Y-%m') = ? ORDER BY date ASC";
    $stmt = $conn->prepare($attendance_sql);
    $stmt->bind_param("is", $selected_employee_id, $current_month);
    $stmt->execute();
    $attendance_result = $stmt->get_result();
    while ($row = $attendance_result->fetch_assoc()) {
        $full_month_records[$row['date']] = array(
            'created' => $row['created'],
            'status' => $row['status']
        );
    }
    $stmt->close();
    $attendance_records = $full_month_records;
}
